...
from bot.strings import Scam as S
...
@command("scam", description=S.DESCRIPTION, usage="f.scam", category="Casino")
async def scam_command(message: Message, args: list[str]):
    ...
    if random.random() < _SUCCESS_CHANCE:
        ...
        embed = tag_embed(discord.Embed(
            ...
            description=S.SUCCESS_DESC.format(scam=random.choice(S.SUCCESS_SCAMS), earned=earned, CURRENCY_EMOJI=CURRENCY_EMOJI),
            color=0x2ECC71,
        ), message.author)
    else:
        ...
        embed = tag_embed(discord.Embed(
            title=S.FAIL_TITLE,
            description=S.FAIL_DESC.format(scam=random.choice(S.FAIL_SCAMS), lost=lost, CURRENCY_EMOJI=CURRENCY_EMOJI),
            color=0xE74C3C,
        ), message.author)
        if new_bal < 0:
